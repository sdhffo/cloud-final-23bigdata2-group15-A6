...
- **成员信息**：
  - 成员1：王公泽-2362160044-代码、PPT、答辩
  - 成员2：沈旭涛-2362160045-代码、实验报告、验证与排错
  - 成员3：徐兢男-2362160046-代码、PPT
  - 成员4：沈浩男-2362160047-环境搭建、代码
- **完成日期**：2026-01-04
...
- 概念 3（镜像版本管理）：
  ...
  - 在本项目中的体现：采用 “应用名：版本号”（如`docker-demo:v1.0`）的标签策略，使用 Docker Registry 存储多版本镜像，通过标签切换实现版本回滚与迭代管理。
...
- 步骤 2：编写基础 Dockerfile 并构建镜像
  ...
  - 输出/结果：
    ...
    成功构建 docker-demo:v1.0 镜像，执行`docker images docker-demo`可查看镜像信息，体积约 97 MB
...
- 步骤 5：多阶段构建实现镜像瘦身
  ...
  - 输出/结果：
    ...
    成功构建瘦身镜像，体积从约 97 MB缩减至约 86.1 MB
